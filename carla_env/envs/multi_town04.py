...
class MultiAgentTown04Env(CarlaEnv):
    ...
    def spawn_vehicles(self) -> None:
        ...
        """
        num_CAV = random.randint(3, 5)
        num_HDV = random.randint(2, 4)
        ...
        spawn_point_cav_main = np.random.choice(selected_main_spawn_points, num_CAV // 2, replace=False)
        spawn_point_cav_merge = np.random.choice(selected_merge_spawn_points, num_CAV - num_CAV // 2, replace=False)
        
        spawn_point_cav_main = list(spawn_point_cav_main)
        spawn_point_cav_merge = list(spawn_point_cav_merge)

        for main in spawn_point_cav_main:
            selected_main_spawn_points.remove(main)
        for merge in spawn_point_cav_merge:
            selected_merge_spawn_points.remove(merge)
        ...
        # Spawn vehicles
        for waypoint in spawn_point_cav_main:
        ...
        for waypoint in spawn_point_cav_merge:
            spawn_point = waypoint.transform
            spawn_point.location.z += 1
            cav = self.world.spawn_actor(vehicle_blueprints[4], spawn_point)
            cav.apply_control(carla.VehicleControl(brake=1.0, steer=0.0))
    ...
    def _reset(self) -> None:
        ...
        self.config["action"]["waypoints"] = self.get_initial_waypoints()
        ...
        for action_type in self.action_type.agents_action_types:
            ...
            action_type.controlled_vehicle.apply_control(carla.VehicleControl(throttle=0.0, brake=1.0))
            ...
            self.colsensor.listen(lambda event: self.collision_data(event, action_type.controlled_vehicle))
    ...
    def _reward(self, action: int) -> float:
        ...
        # Iterate over each vehicle and update the local reward
        for a, vehicle in zip(action, self.cavs):
            self.local_reward[vehicle] = self._agent_reward(vehicle)
            total_reward += self.local_reward[vehicle]
    ...
    def _simulate(self, action: Optional[Action]):        
        for action_type, a in zip(self.action_type.agents_action_types, action):
            action_type.act(a)
    ...
    def _is_terminated(self) -> bool:
        ...
        return all(action_type.is_arrived for action_type in self.action_type.agents_action_types) or any(self.collision_dict[action_type.controlled_vehicle] for action_type in self.action_type.agents_action_types)
